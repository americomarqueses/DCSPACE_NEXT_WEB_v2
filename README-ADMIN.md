# DC Space Admin API (reference)

**Implementation location:** Admin HTTP handlers live in **`DCSPACE_NEXT_API`** (Next.js API routes + Prisma + MongoDB), not inside this web app. Set **`NEXT_PUBLIC_API_URL`** in the web app to the API base URL (e.g. `http://localhost:3001`). See [README-INTEGRATION.md](README-INTEGRATION.md).

The notes below describe **response shapes and query parameters** for admin operations. Prefix paths with the API base URL when calling from the browser or tools.

---

This document was originally written when admin routes were scaffolded inside the web repo. The contracts below still apply; replace relative paths like `/api/admin/...` with `{API_BASE}/api/admin/...`.
